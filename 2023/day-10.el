 ...
 (defun pipes-formatter (pipes)
   (pcase-let* ((`(,loop ,initial) (pipes-loop pipes))
     ...
     (lambda (c i j)
       (let* ((pos `[,i ,j])
              (props (append
                      (cond ((= c ?S) `(display ,(string initial)))
                            ((= c ?.) `(display ,(cat "■"))))
                      (cond ((s. loop pos)
                             `(font-lock-face (:foreground "red")))
                            ((s. inside pos)
                             `(font-lock-face (:foreground "green")))
                            (t `(font-lock-face (:foreground "gray")))))))
         (apply #'propertize (string c) props)))))

 (defun pipes-show (pipes)
   (with-display _
     (print-grid pipes :fmt (pipes-formatter pipes))))

 (pipes-show *test-10-1*)
 (pipes-show *test-10-2*)
 (pipes-show *test-10-3*)
 (pipes-show *test-10-4*)
 (pipes-show *test-10-5*)
 (pipes-show *test-10-6*)
 (pipes-show *input-10*))
